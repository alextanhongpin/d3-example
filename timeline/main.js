...
display()
function display () {
  var extent = brush.extent(items)[0]
  var rects, labels,
    visItems = items.filter(function (d) {return d.start < extent[1] && d.end > extent[0];})

  console.log(extent)
  mini.select('.brush')
    .call(brush.extent(visItems))

  x1.domain(brush.extent(visItems))
  ...
  labels = itemRects.selectAll('text')
    .data(visItems, function (d) { return d.id; })
    .attr('x', function (d) {return x1(Math.max(d.start, extent[0]) + 2);})

  labels.enter().append('text')
    .text(function (d) {return d.id;})
    .attr('x', function (d) {return x1(Math.max(d.start, extent[0]));})
